...
def train() -> None:
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(n_iter):
            ...
            cls_s_label = torch.empty(bs, dtype=torch.long)
            # 源域域标签 = 0
            ...
            if cuda:
                ...
                cls_s_label = cls_s_label.cuda()
                domain_label = domain_label.cuda()

            # 把 1×28×28 广播复制到 3×28×28（三个通道一样）
            # 这两行就是把「真正数据」填到刚才 torch.empty 占好的坑里，完成一次「数据对齐 + 拷贝」。
            input_img.copy_(s_img.repeat(1, 3, 1, 1))
            cls_s_label.copy_(s_label)
            
            # **输入模型 #加入labels算流形学习的graph
            out_cls, out_dom, feature = net(input_img, cls_s_label, alpha=alpha)
            
            # -------- Manifold regularization Source--------
            # TODO 2：让net返回feature extracter的最后一层特征, 利用特征算流形, 增加一个流形 loss_manifold。
            loss_s_mani = mkglmfa_rkhs_loss(feature, cls_s_label, alpha_local=0.5, beta_global=0.5, options=manifold_options)

            # 网络返回：分类 logits，域判别 logits
            err_s_cls = loss_cls(out_cls, cls_s_label)
            err_s_dom = loss_dom(out_dom, domain_label)

            # ---- target ----
            # 目标域前向 + 误差【目标域不训练故障分类器，只要域判别输出】
            t_img, t_label = next(iter_t)  # t_label用于构建流形学习graph
            
            bs = t_img.size(0)
            
            # 先占一块显存/内存
            input_img = torch.empty(bs, 3, image_size, image_size)
            cls_t_label = torch.empty(bs, dtype=torch.long)
            ...
            if cuda:
                ...
                input_img = input_img.cuda()
                cls_t_label = cls_t_label.cuda()
                domain_label = domain_label.cuda()

            input_img.copy_(t_img)
            cls_t_label.copy_(t_label)

            _, out_dom, feature = net(input_img, domain_label, alpha=alpha)
            # -------- Manifold regularization target--------
            # TODO 2
            loss_t_mani = mkglmfa_rkhs_loss(feature, cls_t_label, alpha_local=0.5, beta_global=0.5, options=manifold_options)
            # loss_t_mani = 0   
